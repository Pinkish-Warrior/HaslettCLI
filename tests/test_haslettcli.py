...
def test_init(tmp_path):
    ...
    result = subprocess.run([RUN_SCRIPT, "init"], cwd=tmp_path, capture_output=True, text=True)
...
def test_add_and_list_profile(tmp_path):
    ...
    subprocess.run([RUN_SCRIPT, "init"], cwd=tmp_path)
    ...
    list_result = subprocess.run([RUN_SCRIPT, "list-profiles"], cwd=tmp_path, capture_output=True, text=True)
...
def test_generate_and_cover(tmp_path):
    ...
    subprocess.run(["cp", os.path.join(project_templates_dir, "cv_template.html.j2"), templates_dir])
    # Copy cover_template.txt.j2
    subprocess.run(["cp", os.path.join(project_templates_dir, "cover_template.txt.j2"), templates_dir])
    ...
    with open(dummy_profile_path, 'w') as f:
        yaml.dump(dummy_profile_content, f)
    subprocess.run([RUN_SCRIPT, "add-profile", str(dummy_profile_path)], cwd=tmp_path)
    ...
    cover_result = subprocess.run(
        [RUN_SCRIPT, "cover", "--profile", "dummy_profile.yml", "--job", "TestCo", "--out", "cover.pdf"],
        cwd=tmp_path, capture_output=True, text=True
    )
